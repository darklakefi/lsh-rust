...
fn generate_lsh_rust(inputs: &[u64; 4]) -> String {
    ...
    // let delta1 = inputs[0] as i64 - inputs[2] as i64;
    let delta2: u64 = (inputs[1] as i64 - inputs[3] as i64).abs() as u64;

    // let groupped = [delta1, delta2];

    // let normalized_inputs = normalize_vector(&inputs.iter().map(|&x| x as f64).collect());
    ...
    for i in 0..128 {
        ...
        for &input in lsh_inputs.iter() {
            ...
            // let input_parts = normalize_vector(&split_u64_with_max_bits(input, max_bits).iter().map(|&x| x as f64).collect());
            // println!("input: {}", input);
            let mut input_parts = split_u64_into_weighted_nibbles(input, max_bits);
            input_parts.reverse();
            // println!("input_parts: {:?}", input_parts);
            // let mut check = split_u64_into_weighted_nibbles(input, 64);
            // check.reverse();
            // println!("check:       {:?}", check);

            // if input_parts.iter().sum::<u64>() != check.iter().sum::<u64>() {
            //     println!("Mismatch in input_parts and check sums for input: {}", input);
            // }
            // // println!("input_parts: {:?}", input_parts);


            // let input_bytes = input.to_le_bytes();
            // let mut input_parts = [0u8; 8];
            // input_parts.copy_from_slice(&input_bytes);
...
// slippage = 10,000 = 100%
fn get_hash(is_swap_x_to_y: bool, balance_x: u64, balance_y: u64, input_amount: u64, slippage: u64) -> String {
...
fn main() {
    ...
    for i in 0..10 {
        ...
        for t in 0..9 {
            ...
            (better_balance_x, better_balance_y) = fake_trade_to_x(balance_x, balance_y, front_run_input);
            (worse_balance_x, worse_balance_y) = fake_trade_to_y(balance_x, balance_y, front_run_input);
